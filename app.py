...
@app.route('/api/qrcode/<type>', methods=['GET'])
def bot_qrcode(type):
    if not bot_instance.qrcode:
        return jsonify({"status": "error", "message": "QR code not available"}), 503
    if type=='text':
        return jsonify({
            "qrcode": bot_instance.qrcode.decode('utf-8')
        })  
    elif type=='url':
        qr = qrcode.QRCode(version=1, box_size=10, border=5)
        qr.add_data(bot_instance.qrcode.decode('utf-8'))
        qr.make(fit=True)
        ...
        img = generate_qr(bot_instance.qrcode.decode('utf-8'))
        ...
        return send_file(img_buf, mimetype='image/png', as_attachment=False, download_name="qrcode.png")
        ...
        # Return the image using send_file
        return send_file(img_buf, mimetype='image/png', as_attachment=True, download_name="qrcode.png")
    '''print(f"📷 QR code requested via API {bot_instance.qrcode}")
    return jsonify({
        "qrcode": bot_instance.qrcode.decode('utf-8')
    })'''
